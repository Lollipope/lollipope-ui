...
export default {
  ...
  beforeDestroy: function () {
    const isExist = Array.from(this.parentNode.childNodes).some((n) => {
      let result = n.matches && n.matches(`.${this.className}`);
      return result;
    });
    this.$el.ondragstart = null;
    if (!isExist) {
      removeEvent(
        this.parentNode || document.documentElement,
        "mousedown",
        this.deselect
      );
      removeEvent(
  ...
  methods: {
    ...
    settingAttribute() {
      ...
      this.$el.setAttribute("data-isoverlay-check", `${this.isOverlayCheck}`);
      // 设置对齐元素
    ...
    conflictCheck() {
      ...
      //   }
      // }
      if (this.isOverlayCheck) {
        const nodes = this.$el.parentNode.childNodes; // 获取当前父节点下所有子节点
        for (const item of nodes) {
          if (
            item.className !== undefined &&
            !item.className.includes(this.classNameActive) &&
            item.getAttribute("data-isoverlay-check") !== null &&
            item.getAttribute("data-isoverlay-check") !== "false"
          ) {
            const tw = item.offsetWidth;
            const th = item.offsetHeight;
            // 正则获取left与right
            ...
            //     Math.abs(left - tl) < 1 &&
            //     Math.abs(width - tw) < 1 &&
            //     Math.abs(height - th) < 1,
            // )

            // 完全重叠
            // const isOverlay =
            //   top === tt && left === tl && tw === width && th === height
